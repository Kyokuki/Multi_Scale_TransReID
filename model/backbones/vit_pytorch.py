...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, local_feature=False, sie_xishu=1.0, svd=False):
        super().__init__()
        ...
        self.cam_num = camera
        self.view_num = view
        self.sie_xishu = sie_xishu  # 控制SIE权重的超参数
        self.svd = svd
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if k in self.state_dict() and self.state_dict()[k].shape == v.shape:
                ...
                print(f"Skipping {k}: shape mismatch {v.shape if k in param_dict else 'N/A'} vs {self.state_dict()[k].shape if k in self.state_dict() else 'Not in model'}")
...
def vit_base_patch16_224_TransReID(img_size=(128, 64), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, svd=False, **kwargs):
    model = TransReID(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6),  sie_xishu=sie_xishu, svd=svd, local_feature=local_feature, **kwargs)

    return model


def vit_base_patch32_224_TransReID(img_size=(128, 64), stride_size=32, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, svd=False, **kwargs):
    model = TransReID(
...
def svd_fdr(x):
    # SVD feature dimension reduction
    svd_k = 8

    # 对每个样本的[num_patches, D]矩阵进行SVD
    U, S, Vh = torch.linalg.svd(x.float(), full_matrices=False)

    # 截断奇异值（保留前k个）
    S_trunc = torch.zeros_like(S)
    S_trunc[:, :svd_k] = S[:, :svd_k]

    # 重构低秩矩阵
    diag_S = torch.diag_embed(S_trunc)  # [B, P, P]
    svd_feat = U @ diag_S @ Vh  # [B, P, D]
